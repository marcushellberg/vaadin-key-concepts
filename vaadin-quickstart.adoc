= Vaadin key concepts and quickstart
...
=== 1. (Almost) Everything is a component
...
Add a listerner to a button with `addClickListener()` or get notified of changed selection in a select with `addValueChangeListener()`. 
...
== Creating new components through composition
...
== Extending the framework with custom JavaScript components
